...
#loading csv
data <- read.csv("C:\\Users\\Michaela\\OneDrive\\Desktop\\CTA ABD PEL MK - ACR FINAL.csv", header = TRUE)
...
data$intervention_bleeding <- NULL
data$days_to_intervention <- NULL
data$pulse_pressure <- NULL


#report median and Q1Q3
# Variables for descriptive statistics
descriptive_stats <- c("CTA_result", "exam_age", "Hgb", "Hct", "HR", "systolic_BP", "diastolic_BP", "MAP", "PT", "INR")
...
variables <- c("exam_age", "Hgb", "Hct", "HR","systolic_BP", "diastolic_BP", "MAP", "PT", "INR")
results <- data.frame(
  Variable = character(),
  Statistic = numeric(),
  P_Value = numeric(),
...
for(var in variables) {
  ...
  normality <- ifelse(test_result$p.value > 0.05, "Normal", "Not Normal")
...
table_data <- table(data$Anticoagulation, data$CTA_result)
...
logistic <- glm(CTA_result~., data=data, family="binomial")
summary(logistic)
...
#check nonlinear relationships
#NOT USING FOR ACR
# Load the mgcv package
library(mgcv)

# Fit a GAM for Hct
gam_model <- gam(CTA_result ~ s(Hct), family = "binomial", data = data)

# Summary of the GAM model
summary(gam_model)

# Generate predictions for plotting
new_data$predicted_prob <- predict(gam_model, newdata = new_data, type = "response")

# Plot the relationship
ggplot(new_data, aes(x = Hct, y = predicted_prob)) +
  geom_line(color = "green", size = 1) +
  labs(title = "GAM: Hct and CTA Result",
       x = "Hct",
       y = "Predicted Probability of Positive CTA") +
  theme_minimal()
